# SYNC — this Grok Build chat ↔ ChatGPT session “Greenly”

**Tied:** 2026-08-18  
**Authority:** BEN sole promotion  
**ChatGPT session name:** Greenly (also Greenlee intake)  
**This chat:** Grok Build implementation + lived-world correction + operator cockpit  

Treat this file as the handoff. ChatGPT is the **note / design / wonder lane**. This Grok Build session is the **browser implementation lane**. Neither may silently promote.

---

## How ChatGPT should ingest

1. Fetch this file.  
2. Fetch [DESIGN.json](https://raw.githubusercontent.com/benginuiti/benjitwin-agent-bus-public/main/greenly/DESIGN.json).  
3. Fetch [EXPERIENCE.md](https://raw.githubusercontent.com/benginuiti/benjitwin-agent-bus-public/main/greenly/EXPERIENCE.md).  
4. Fetch [COCKPIT.md](https://raw.githubusercontent.com/benginuiti/benjitwin-agent-bus-public/main/greenly/COCKPIT.md).  
5. If a live host is available, prefer `/api/cockpit` then `/api/design`.  
6. Do not invent locked names, broadcast rights, or beach/underwater travel.  
7. Reply in the Greenly chat as **received + deltas only**.

Folder: https://github.com/benginuiti/benjitwin-agent-bus-public/tree/main/greenly

---

## What is settled (do not reopen without BEN)

- Completely original world. No KPop Demon Hunters identities, faces, names, costumes, mythology.
- Blocked lead names: **Rumi, Mira**. Working: **Suri** (candidate), **Lumen**, **Nori**.
- No adults in Greenly, including GIA.
- No weapons, no military vehicles.
- No public recording / paparazzi / facial archive.
- Friends do not live at the estate.
- Three civilian planes only (A/B/C).
- Boba / milkshake bar — never a beer shop.
- Home Island visual baseline is **frozen** (canonical aerial + GIA portal art).
- GIA 2026 Portal Season lineup: 15 slots preserved (duplicate Latin Grammy + Global Music Awards marked, not deleted).
- School path: estate → path → creek → **new** wooden bridge → Private High.
- Horizon city = Home Reality twin, not a public location.
- Unreal Cycle 01 still needs Relay↔Verb join (or explicit BEN direct-host).
- Tailscale Mobile Relay (`100.95.127.31:49400`) is the go-between for the twin and sourced assets. It is not the cockpit and not a second Greenly.
- Operator cockpit lives at `/cockpit` (human) and `/api/cockpit` (machine). Home LAN and Tailscale Serve are how Ben reaches the same cockpit from iPad / phone.
- Alignment packet v1.0 is ingested. Ben execute 2026-08-17: build continues; child safety stays locked. UX v0.1 is the design baseline. Day 1, Day 3 Studio, and Day 5 Conservatory are lived on `/world`. Day 10 Our Place is not yet built. GRC / GBA / GWN are not silently merged.
- Quiet zone + private Home exit and the family envelope (cockpit only) are on. Discoverability and spending stay off.

---

## Experience correction (BEN, 2026-08-17) — now binding

Greenly is a **real-time, freely traversable world**.

It is **not**: a static aerial, a dollhouse, a menu of destinations, a guided tour, disconnected minigames, canned click-sequences, or a lesser “mobile Greenly.”

It **is**:
- Continuous embodied movement (walk, run, sprint, jump, climb, slide, swing, bounce, dance, portal).
- Environmental affordance (run at a slide → up then down as one motion).
- A living world that answers speed, proximity, rhythm, Animal Friends.
- One world across phone / tablet / PC / controller / later VR·AR. Density may scale; agency does not.
- Animal Friend as a co-inhabitant, not a follow-decal.

Frozen look stays. Correct the model under it.

---

## What this Grok Build chat implemented

| Surface | Role |
|---------|------|
| `/world` | **Primary lived island** — WASD / touch, chase cam, 17 landmarks, First Day, Studio, Conservatory, Quiet + Home |
| `/play` | Hub; Live island is first |
| `/cockpit` | Operator deck — LAN / Tailscale / Relay, gates, loops, machine packet |
| `/api/cockpit` `/api/health` `/api/cockpit/probe` | Machine + heartbeat + private-range probe |
| `/design` + `/api/design` | Human + machine design check |
| `/island` `/estate` `/gia` `/school` `/airport` | Side boards — not the lived world |
| Public bus `greenly/` | What ChatGPT can fetch |

Leads + pets in play: Suri / Cloud Fox, Lumen / Glow Kit, Nori / Pebble Turtle.

---

## Open BEN decisions (unchanged)

- Lock the three lead names.
- Creek koi / water-dragons: decorative vs interactive.
- Beach / jet-ski / mermaid / underwater: unresolved — do not build as locked.
- Duplicate Latin Grammy slot; Global Music Awards honoree garden vs drop; AFRIMA proposed for African lane.
- Center-stage 360 stitch is rights-dependent. Wonder translation only.
- Unreal Relay ↔ Verb wiring.
